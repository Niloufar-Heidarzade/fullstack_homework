...
import { setSelectedTaskId, toggleTaskCompleted } from "../redux/taskListslice";
...
function TaskCard({isFirst ,id ,title , description , completed , important , deadline , directory}) {
  ...
  return (
    <div className={isFirst ? "card-first" : "card"}>
      ...
      <div className="date">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6" className={"first-date-icon"}>
      ...
      <div className="carrd-bottom">
        <button className={completed ? "completed-button" : "uncompleted-button"} onClick={() => dispatch(toggleTaskCompleted(id))}>
        ...
        <div className="card-icons">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 512"  className={isFirst ? "first-options" : "options-icon"} onClick={() => {
            dispatch(setSelectedTaskId(id))
